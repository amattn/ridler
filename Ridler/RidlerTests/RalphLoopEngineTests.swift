...
final class RalphLoopEngineTests: XCTestCase {
    ...
    // MARK: - Auto-Retry Tests (US-028)

    func testAutoRetryRetriesOnCrashWhenEnabled() throws {
        let stories = [
            UserStory(id: "US-001", title: "First", description: "d", priority: 1, acceptanceCriteria: ["a"]),
        ]
        var project = try createTestProject(stories: stories)
        project.autoRetryEnabled = true

        var spawnCount = 0
        var latestMockPM: MockProcessManager?
        let engine = RalphLoopEngine(
            prdStore: FileSystemPRDStore(),
            processManagerFactory: {
                let pm = MockProcessManager()
        ...
        // First spawn happened
        XCTAssertEqual(spawnCount, 1)

        // Simulate crash
        latestMockPM?.sendExit(code: 1, stderr: "crash")

        // Wait for retry (backoff: 2s for first retry)
        let retryExpectation = XCTestExpectation(description: "Retry spawned")
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
            retryExpectation.fulfill()
        }
        wait(for: [retryExpectation], timeout: 5.0)

        // Second spawn should have happened (retry)
        XCTAssertEqual(spawnCount, 2, "Should retry after crash")
        XCTAssertTrue(logMessages.contains(where: { $0.contains("Auto-retry 1/3") }), "Should log retry event")
    }

    func testAutoRetryDisabledGoesToErrorImmediately() throws {
        let stories = [
            UserStory(id: "US-001", title: "First", description: "d", priority: 1, acceptanceCriteria: ["a"]),
        ]
        var project = try createTestProject(stories: stories)
        project.autoRetryEnabled = false
        ...
        wait(for: [errorExpectation], timeout: 3.0)
        // No retry should happen — error immediately
    }

    func testAutoRetryExhaustsAndTransitionsToError() throws {
        let stories = [
            UserStory(id: "US-001", title: "First", description: "d", priority: 1, acceptanceCriteria: ["a"]),
        ]
        var project = try createTestProject(stories: stories)
        ...
        var gotError = false
        let errorExpectation = XCTestExpectation(description: "Error after exhaustion")
        engine.onStateChange = { state in
            if state == .error && !gotError {
                gotError = true
                errorExpectation.fulfill()
            }
        }
        ...
        // We need to crash 4 times total (1 original + 3 retries) for exhaustion
        // Retry backoff: 2s, 8s, 18s — too long for tests
        // We'll verify the first crash triggers retry, then manually simulate the rest
        // by sending exits after each retry respawns

        // crash 1 → triggers retry 1 (2s backoff)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
            latestMockPM?.sendExit(code: 1, stderr: "crash 1")
        }
        // crash 2 after retry 1 respawns (2s backoff)
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.5) {
            latestMockPM?.sendExit(code: 1, stderr: "crash 2")
        }
        // crash 3 after retry 2 respawns (8s backoff)
        DispatchQueue.main.asyncAfter(deadline: .now() + 11.0) {
            latestMockPM?.sendExit(code: 1, stderr: "crash 3")
        }
        // crash 4 after retry 3 respawns (18s backoff)
        DispatchQueue.main.asyncAfter(deadline: .now() + 29.5) {
            latestMockPM?.sendExit(code: 1, stderr: "crash 4 - exhausted")
        }

        wait(for: [errorExpectation], timeout: 35.0)

        XCTAssertEqual(spawnCount, 4, "Should spawn 4 times total (1 original + 3 retries)")
        XCTAssertTrue(logMessages.contains(where: { $0.contains("Auto-retry exhausted") }), "Should log retry exhaustion")
    }

    func testAutoRetryLogMessagesShownInLog() throws {
        let stories = [
            UserStory(id: "US-001", title: "First", description: "d", priority: 1, acceptanceCriteria: ["a"]),
        ]
        var project = try createTestProject(stories: stories)
        ...
        // Crash the process
        latestMockPM?.sendExit(code: 1, stderr: "something broke")

        // Wait for retry log message
        let logExpectation = XCTestExpectation(description: "Retry log appeared")
        ...
        XCTAssertTrue(logMessages.contains(where: { $0.contains("Auto-retry") }), "Retry event should be shown in log")
        XCTAssertTrue(logMessages.contains(where: { $0.contains("retrying in") }), "Log should include backoff info")
    }

    func testAutoRetryDefaultOff() {
        let project = PRDProject(name: "Test")
        XCTAssertFalse(project.autoRetryEnabled, "Auto-retry should default to off")
    }

    func testAutoRetryResetOnSuccess() throws {
        let stories = [
            UserStory(id: "US-001", title: "First", description: "d", priority: 1, acceptanceCriteria: ["a"]),
            UserStory(id: "US-002", title: "Second", description: "d", priority: 2, acceptanceCriteria: ["b"]),
        ]
        ...
        // Crash once, then retry succeeds
        latestMockPM?.sendExit(code: 1, stderr: "crash")
        ...
        XCTAssertEqual(spawnCount, 2, "Should have retried once")

        // Now the retry succeeds
        let pausedExpectation = XCTestExpectation(description: "Paused after success")
        engine.onStateChange = { state in
            if state == .paused {
                pausedExpectation.fulfill()
            }
        ...
        latestMockPM?.sendExit(code: 0)
        wait(for: [pausedExpectation], timeout: 3.0)

        // Success should have been processed normally (paused because pauseAfterStory)
    }

    func testUpdateAutoRetryAtRuntime() throws {
        let stories = [
            UserStory(id: "US-001", title: "First", description: "d", priority: 1, acceptanceCriteria: ["a"]),
        ]
        var project = try createTestProject(stories: stories)
        ...
        // Enable auto-retry at runtime
        engine.updateAutoRetry(true)

        // Crash — should now retry instead of erroring
        var logMessages: [String] = []
        engine.onLogEntry = { entry, _ in
            if entry.type == .system {
                logMessages.append(entry.content)
        ...
        mockPM?.sendExit(code: 1, stderr: "crash after enable")

        let retryLogExpectation = XCTestExpectation(description: "Retry log appeared")
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            retryLogExpectation.fulfill()
        }
        wait(for: [retryLogExpectation], timeout: 2.0)

        XCTAssertTrue(logMessages.contains(where: { $0.contains("Auto-retry") }), "Should retry after runtime enable")
    }
